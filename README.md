...
- [x] Country/slug/iso 정합성 검사 & 자동 정규화(시딩/관리용 커맨드 포함)
- [x] 국가 데이터 “재시딩 안전성” 강화(중복 방지, `--force` 동작 명확화)
- [x] 게시물 slug 정책 고도화(중복 처리, 한글→slug 규칙 고정, 변경 시 리다이렉트 옵션)
- [x] Admin에서 Country/Posts 데이터 검증(iso 길이, slug 중복, alias 형식) 경고 표시
- [x] 에러/로그 기반 운영 점검(DisallowedHost, static/media, seed/migrate)

### 🎛️ Phase 2 — UX 개선 (방문자/관리자 체감)
- [x] 국가 클릭 시 로딩/실패 상태 UI(스켈레톤, “해당 국가 콘텐츠 없음” 안내)
- [x] 검색 UX 강화(자동완성/최근 검색, 결과 하이라이트, 빈 결과 메시지)
- [ ] 게시물 목록 정렬 옵션(최신/인기/카테고리/태그)
- [x] 라이트박스 UX 개선(키보드 방향키 이동, 이미지 카운터, 썸네일 스트립)
- [x] 반응형/모바일 최적화(보드 레이아웃, 터치 UX)
